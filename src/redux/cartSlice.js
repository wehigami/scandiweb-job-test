...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    setCartIncrement(state, action) {
      state.cart[idIndex(state, action)].quantity++; // increment quantity
    },
    setCartDecrement(state, action) {
      state.cart[idIndex(state, action)].quantity--; // decrement quantity
    },
    setCartSplice(state, action) {
      state.cart.splice(idIndex(state, action), 1); // remove item from cart
    },
